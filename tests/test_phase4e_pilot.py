...
def test_spend_below_baseline_refuses_before_transport(
    ...
    plan = tmp_path / "pilot-plan-v1" / "plan.json"
    ...
    with pytest.raises(corpus.CorpusError, match="cumulative spend baseline"):
        pilot.run_pilot(
            plan,
            None,
            tmp_path / "pilot-work-v1",
            tmp_path / "pilot-report-v1",
            root,
            allow_network=True,
            transport=transport,
            counter=_Counter(),
...
def test_pilot_without_opt_in_constructs_no_socket(
    ...
    with pytest.raises(corpus.CorpusError, match="literal --allow-network"):
        pilot.run_pilot(
            tmp_path / "pilot-plan-v1" / "plan.json",
            None,
            tmp_path / "pilot-work-v1",
            tmp_path / "pilot-report-v1",
            tmp_path / "root",
...
def test_fake_transport_passes_with_diagnostic_semantic_disagreement(
    ...
    root = _baseline_root(tmp_path / "root")
    plan_path = tmp_path / "pilot-plan-v1" / "plan.json"
    ...
    report_path = pilot.run_pilot(
        plan_path,
        None,
        tmp_path / "pilot-work-pass",
        tmp_path / "pilot-report-pass",
        root,
        allow_network=True,
        transport=transport,
        counter=_Counter(),
...
def test_fake_transport_resolves_all_140_without_wilson_stop(
    ...
    report = json.loads(
        pilot.run_pilot(
            plan_path,
            None,
            tmp_path / "pilot-work-fail",
            tmp_path / "pilot-report-fail",
            root,
            allow_network=True,
            transport=transport,
            counter=_Counter(),
...
def test_uncertain_transport_is_terminal_and_inconclusive(
    ...
    work = tmp_path / "pilot-work-retry"
    first = json.loads(
        pilot.run_pilot(
            plan_path,
            None,
            work,
            tmp_path / "pilot-report-first",
            root,
            allow_network=True,
            transport=transport,
            counter=_Counter(),
...
def test_resume_restores_persisted_diagnostics_without_another_call(
    ...
    work = tmp_path / "pilot-work-diagnostics-resume"
    first = json.loads(
        pilot.run_pilot(
            plan_path,
            None,
            work,
            tmp_path / "pilot-report-diagnostics-first",
            root,
            allow_network=True,
    ...
    second = json.loads(
        pilot.run_pilot(
            ...
            None,
            work,
            tmp_path / "pilot-report-diagnostics-second",
            root,
            allow_network=True,
...
def test_orphan_diagnostics_fail_closed_without_transport(
    ...
    plan_path = tmp_path / "pilot-plan-v1" / "plan.json"
    pilot.write_pilot_plan(plan_path)
    work = tmp_path / "pilot-work-orphan-diagnostics"
    ...
    report = json.loads(
        pilot.run_pilot(
            ...
            None,
            work,
            tmp_path / "pilot-report-orphan-diagnostics",
            root,
            allow_network=True,
            transport=forbidden_transport,
            counter=_Counter(),
