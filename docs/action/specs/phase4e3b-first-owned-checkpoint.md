...
1. Bind the live execution to one reviewed private layout below the configured external Phase 4E state root: the sealed packet input, exact sealed report, create-only embedding capsule and create-only training-capsule name. The v4 CLI may retain the existing explicit `--snapshot`, `--embeddings`, `--output-parent` and `--output-name` arguments for v2/v3 compatibility, but for packet v4 it must validate each path against the canonical configured private root and exact reviewed logical names before opening it. Repository, Git-common, worktree, synchronized, symlinked and arbitrary caller-selected destinations fail closed. Tracked docs and reports use only logical artifact names and digests.
...
1. A valid v4 preflight proves the packet/report bindings, accepted counts, 943/609 locale composition, fixed source policy and complete lineage before snapshot access or output creation.
