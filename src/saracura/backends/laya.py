...
from saracura.laya_snapshot import LayaSnapshotError, LayaSnapshotReceipt, verify_laya_snapshot
...
def render_laya_choice(
    ...
    if any(len(item) > MAX_DESCRIPTION_TOKENS for item in descriptions):
        raise SaracuraError(
            ErrorCode.CAPACITY_EXCEEDED,
            "Complete universal input does not fit the Laya envelope.",
            "/questions/*/criteria",
...
class LayaUniversalBackend:
    ...
    def __init__(
        ...
        *,
        ...
        try:
            ...
            receipt = verify_laya_snapshot(Path(model_snapshot))
            try:
            ...
            receipt.assert_live()
            self._model_object = model.to(device).eval()
            self._torch = torch
            ...
            self._reference = ModelReference(
                id=receipt.candidate.model_id,
                revision=receipt.candidate.model_revision,
                checkpoint_sha256=receipt.candidate.checkpoint_sha256,
            )
    ...
    def score_universal_choice(
        ...
        try:
            self._receipt.assert_live()
            rendered = render_laya_choice(self._tokenizer, request, question)
            torch = self._torch
            ...
            self._receipt.assert_live()
            return ScoredChoice(
                question_id=question.id, raw_scores=scores, input_tokens=rendered.input_tokens
            )
